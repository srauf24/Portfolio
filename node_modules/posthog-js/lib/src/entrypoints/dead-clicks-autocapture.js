...
var LazyLoadedDeadClicksAutocapture = /** @class */ (function () {
    function LazyLoadedDeadClicksAutocapture(instance, config) {
        ...
        this._onClick = function (event) {
            ...
            if (!isNull(click) && !_this._ignoreClick(click)) {
                _this._clicks.push(click);
            }
            if (_this._clicks.length && isUndefined(_this._checkClickTimer)) {
                _this._checkClickTimer = assignableWindow.setTimeout(function () {
                    _this._checkClicks();
                }, 1000);
            }
        };
    ...
    LazyLoadedDeadClicksAutocapture.prototype._startMutationObserver = function (observerTarget) {
        ...
        if (!this._mutationObserver) {
            var NativeMutationObserver = getNativeMutationObserverImplementation(assignableWindow);
            this._mutationObserver = new NativeMutationObserver(function (mutations) {
                _this.onMutation(mutations);
            });
    ...
    LazyLoadedDeadClicksAutocapture.prototype.onMutation = function (_mutations) {
    ...
    LazyLoadedDeadClicksAutocapture.prototype._checkClicks = function () {
        ...
        try {
            for (var clicksToCheck_1 = __values(clicksToCheck), clicksToCheck_1_1 = clicksToCheck_1.next(); !clicksToCheck_1_1.done; clicksToCheck_1_1 = clicksToCheck_1.next()) {
                ...
                var hadScroll = isNumber(click.scrollDelayMs) && click.scrollDelayMs < this._config.scroll_threshold_ms;
                var hadMutation = isNumber(click.mutationDelayMs) && click.mutationDelayMs < this._config.mutation_threshold_ms;
                var hadSelectionChange = isNumber(click.selectionChangedDelayMs) &&
        ...
        if (this._clicks.length && isUndefined(this._checkClickTimer)) {
            this._checkClickTimer = assignableWindow.setTimeout(function () {
                _this._checkClicks();
            }, 1000);
        }
    };
    LazyLoadedDeadClicksAutocapture.prototype._captureDeadClick = function (click, properties) {
        ...
        this.instance.capture('$dead_click', __assign(__assign(__assign({}, properties), autocapturePropertiesForElement(click.node, {
