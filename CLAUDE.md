...
This is a personal portfolio website for Samee Rauf, a Software Engineer. The site is a **static single-page application** built with vanilla HTML/CSS/JavaScript, with no build process required. The primary content lives in `index.html` as a self-contained document.

## Architecture

### Single-Page Static Site
- **Main file**: `index.html` - Contains all HTML structure, embedded CSS (in `<style>` tags), and semantic markup
- **Styling**: Custom CSS with CSS variables for theming (dark mode design with accent colors)
- **Structure**: Traditional multi-section layout (Hero, About, Experience, Projects, Skills, Contact)
- **Analytics**: PostHog integration via `posthog-js` package
...
│   └── Samee_Rauf_Resume.pdf
├── js/                 # Legacy Bootstrap scripts (not actively used in main site)
│   └── scripts.js
├── frontend/           # Unused React setup (appears to be from initial scaffolding)
└── node_modules/       # Only contains posthog-js dependency
```

### Key Design Patterns
- **CSS Variables**: All colors and theme values defined in `:root` selector for easy customization
- **Component-style CSS**: Classes follow BEM-like conventions (e.g., `.experience-card`, `.project-card`, `.tech-tag`)
- **Responsive Design**: Mobile-first with `clamp()` for fluid typography and media queries for layout adjustments
- **Animations**: CSS-based fade-up animations with staggered delays using `.animate` classes

## Development

### Running Locally
This is a static site with no build step. Simply open `index.html` in a browser:
```bash
...
### Dependencies
```bash
npm install  # Only installs posthog-js
```

### Making Content Changes
All content edits happen directly in `index.html`:
- **Personal info**: Lines 559-576 (hero section)
- **Experience**: Lines 624-693 (experience cards)
- **Projects**: Lines 696-811 (project cards)
- **Skills**: Lines 814-862 (skills grid)
- **Contact**: Lines 866-877 (footer)

### Styling Modifications
1. **Theme colors**: Modify CSS variables in `:root` (lines 11-21)
2. **Typography**: Font families loaded from Google Fonts (lines 7-9)
3. **Layout**: All component styles are embedded in `<style>` tag (lines 10-541)

## Important Notes

### Deployment
- This site appears to be deployed to GitHub Pages (presence of `CNAME` file)
- The `CNAME` file contains the custom domain configuration
- No build artifacts needed - `index.html` is the production-ready file

### Unused/Legacy Code
- `frontend/` directory contains an unused React setup - can be ignored
- `js/scripts.js` contains Bootstrap code not used in the current site implementation
- The site previously used Bootstrap but has been rewritten with custom CSS

### Resume Asset
- Resume PDF located at `/assets/Samee_Rauf_Resume.pdf`
- Referenced in index.html:572 as download link
- Also duplicated in `/frontend/Samee_Rauf_Resume.pdf`

### Analytics Setup
- PostHog is imported but initialization code appears incomplete (js/scripts.js:35-37)
- If PostHog needs to be fully configured, add initialization in `<script>` tag or external JS file
